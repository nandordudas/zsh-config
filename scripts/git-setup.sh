...
# machine's CPU core count (fetch.parallel, index.threads) at generation time.
# ═══════════════════════════════════════════════════════════════════════════════
...
[core]
	...
	fsmonitor = true
	pager = delta
	# preloadindex is a git default since 2.1 — listed for explicitness
	preloadindex = true
	quotePath = false
	safeCRLF = warn
...
[log]
	...
	showSignature = false
[maintenance]
	# auto and autodetach are git defaults — listed here for explicitness
	auto = true
	autodetach = true
	strategy = incremental
	# Note: per-task schedules (e.g. [maintenance "gc"] schedule = daily) are not set here;
	# `strategy = incremental` already configures the recommended task cadence.
[pull]
	rebase = merges
[blame]
...
[gc]
	# autodetach is a git default since 2.6 — listed for explicitness
	autodetach = true
	cruftPacks = true
[grep]
	column = true
...
[rerere]
	autoUpdate = true
	enabled = true  # must be set explicitly; autoUpdate is a no-op without it
...
[delta]
	...
	navigate = true
	# auto pages only when output exceeds terminal height; always forces pager even for 2-line diffs
	paging = auto
...
[alias]
	...
	# ━━━━━━ MONOREPO ━━━━━━
	# Note: `cd` in the !-subshell does not affect your current shell.
	# Use a shell function in .zshrc if you need real navigation.
	workspace-status = "!f() { git -C packages/$1 status; }; f"
