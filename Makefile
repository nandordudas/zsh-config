...
# Build Docker image with versions from versions.env
docker-build:
	docker build \
...
versions:
	@echo "Tool versions from versions.env:"
	@grep -v '^#' versions.env | grep -v '^$$'

.PHONY: help
help:
	@echo "zsh-config Makefile targets:"
	@echo "  make docker-build     Build Docker image with versions from versions.env"
